...
evntBus.$on('update_customer', (customer) => {
  const vm = this;
  if (this.customer) {
    // Call API method to fetch membership card name
    frappe.call({
      method: 'posawesome.posawesome.api.posapp.get_membership_card_names_customer',
      args: {
        customer: this.customer
      },
      callback: (r) => {
        if (r.message && r.message.length > 0) {
          const membershipCards = r.message; // Assign the entire response
          vm.membershipcards = membershipCards;
        } else {
          console.log('No membership cards found');
          vm.membershipcards = [];
        }
      },
    });
